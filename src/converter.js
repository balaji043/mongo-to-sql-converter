...
const parseStructure = (originalQuery) => {
	...
	if (index === -1) {
		throw Error('Wrong format index -1');
	}
	const prefix = originalQuery.slice(0, index);
	if (prefix !== 'db') {
		throw Error('Wrong format db');
	}
	...
	index = rest.indexOf('.');
	if (index === -1) {
		throw Error('Wrong format collection name');
	}
...
const parseStatements = (originalStatements) => {
	...
	if (
		originalStatements[0] !== '(' ||
		originalStatements[originalStatements.length - 1] !== ')'
	) {
	...
	if (isLimitIncluded) {
		...
		console.log({ originalStatementsAfterLimitRemoved: originalStatements });
	}
	...
	const preparedForObjectParsing =
		'[' + originalStatements.slice(1, originalStatements.length - 1) + ']';
	...
	const whereClausePrepared = whereParsed.parts.reduce(
		(prev, curr) => [...prev, prepareWhereClause(curr)],
		[]
	);
...
const buildSQL = (describedQuery, removeUnderscoreBeforeID) => {
	...
	const whereClauseSQL = whereClausePrepared
		.reduce(
			(prev, curr) => [
				...prev,
				buildWhereElement(curr, removeUnderscoreBeforeID),
			],
			[]
		)
		.join(' AND ');
	...
	const select =
		'SELECT ' +
		(selectClausePrepared.length > 0 ? selectClausePrepared.join(', ') : '*');
	const from = 'FROM ' + fromClausePrepared;
	const where = 'WHERE ' + whereClauseSQL;
	const limit = limitNumber ? 'LIMIT ' + limitNumber : '';
